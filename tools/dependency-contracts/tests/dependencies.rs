...
use std::fs;
use support::{check_all, coverage, crate_dependencies, Fixture, Kind, Problem, TestResult};
...
fn denied(fixture: &Fixture, from: &str, to: &str, kind: Kind) -> TestResult {
    ...
    assert!(
        matches!(
            &problem,
            Problem::Forbidden { rule, package, dependency, kind: actual, .. }
                if rule == expected.name && package == from && dependency == to && *actual == kind
        ),
...
forbidden_probe!(
    renamed_runtime_dependency_is_rejected,
    "dependencies",
    Normal,
    "false"
);
forbidden_probe!(
    inactive_optional_dependency_is_rejected,
    "dependencies",
    Normal,
    "true"
);
forbidden_probe!(
    development_dependency_is_rejected,
    "dev-dependencies",
    Dev,
    "false"
);
forbidden_probe!(
    build_dependency_without_build_script_is_rejected,
    "build-dependencies",
    Build,
    "false"
);
forbidden_probe!(
    other_platform_dependency_is_rejected,
    "target.'cfg(windows)'.dependencies",
    Normal,
    "false"
);
forbidden_probe!(
    other_platform_build_dependency_is_rejected,
...
fn application_cannot_add_a_concrete_adapter() -> TestResult {
    ...
    fixture.dependency(
        "application",
        "dependencies",
        "task-infrastructure = { path = \"../infrastructure\" }",
    )?;
    denied(
        &fixture,
        "task-application",
...
fn same_name_from_a_different_path_is_rejected() -> TestResult {
    ...
    fixture.dependency(
        "application",
        "dependencies",
        "imposter = { package = \"task-domain\", path = \"../../../probe\" }",
    )?;
    fixture.compile_mutation()?;
    ...
    assert_eq!(
        problem,
        Problem::WrongSource {
            package: "task-application".into(),
            dependency: "task-domain".into(),
        }
    );
    fixture.pass(&format!("Expected rejection: {problem}"))
}
...
fn new_workspace_member_requires_a_contract() -> TestResult {
    ...
    fixture.compile_mutation()?;
    let problem =
        check_all(&fixture.metadata()?, RULES).expect_err("Unclassified member was accepted");
...
fn missing_duplicate_and_misspelled_rules_are_rejected() -> TestResult {
    ...
    let metadata = fixture.metadata()?;
    assert!(matches!(
        coverage(&metadata, &RULES[..4]),
        Err(Problem::Coverage(_))
    ));
...
fn empty_or_malformed_metadata_never_passes() {
    for metadata in [
        ...
        json!({}),
        json!({"version": 2}),
        json!({"version": 1, "workspace_members": ["missing"], "packages": []}),
    ] {
        assert!(matches!(
            check_all(&metadata, RULES),
            Err(Problem::InvalidMetadata(_))
        ));
    }
    assert!(matches!(
        check_all(
            &json!({"version": 1, "workspace_members": [], "packages": []}),
            &[]
        ),
        Err(Problem::Coverage(_))
    ));
...
#[test]
fn remote_source_cannot_impersonate_a_workspace_package() -> TestResult {
    let fixture = Fixture::new("remote_source_cannot_impersonate_a_workspace_package")?;
    let original = fixture.metadata()?;
    // Evaluator unit controls, not network-based dependency resolution tests.
    for source in ["registry+example", "git+example", "future+example"] {
...
fn compiler_rejects_fully_qualified_outer_type_without_dependency() -> TestResult {
    ...
    assert!(
        ...
        "Expected compiler failure: {output}"
    );
    let diagnosed = output
        .lines()
        ...
        .any(|entry| {
            entry["reason"] == "compiler-message"
                ...
                && entry["message"]["message"]
                    .as_str()
                    .is_some_and(|text| text.contains("task_infrastructure"))
        });
