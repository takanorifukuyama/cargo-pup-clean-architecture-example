...
fn analyze(name: &str, source: &Path, crate_name: &str, cfg: &[&str]) -> TestResult<Graph> {
    ...
    let driver = std::env::var_os("LAYER_DRIVER").map(PathBuf::from).unwrap_or_else(|| {
        Path::new(env!("CARGO_MANIFEST_DIR")).join("target/debug").join(format!("layer-driver{}", std::env::consts::EXE_SUFFIX))
    });
    if !driver.is_file() { return Err("Build layer-driver with the pinned toolchain first".into()); }
    let (code, sysroot) = execute(
        Command::new("rustup").args(["run", TOOLCHAIN.trim(), "rustc", "--print", "sysroot"]),
        &logs.join("sysroot.log"),
    )?;
    if code != Some(0) { return Err(format!("Toolchain unavailable: {sysroot}").into()); }
    let output = project.0.join("graph.txt");
    for (phase, program) in [("baseline", Path::new("rustc")), ("collector", driver.as_path())] {
        let mut command = Command::new("rustup");
        command.args(["run", TOOLCHAIN.trim()]).arg(program);
        command.args(["--edition=2021", "--crate-type=lib", "--emit=metadata", "--crate-name"])
            .arg(crate_name).arg("--sysroot").arg(sysroot.trim())
            .arg("--out-dir").arg(&project.0).arg(source);
        for feature in cfg { command.arg("--cfg").arg(format!("feature={feature:?}")); }
        command.env("LAYER_GRAPH_OUT", &output);
        let (code, text) = execute(&mut command, &logs.join(format!("{phase}.log")))?;
        if code != Some(0) { return Err(format!("{name}: {phase} failed (not an architecture rejection):\n{text}").into()); }
    }
    let text = fs::read_to_string(output)?;
    let graph = Graph::decode(&text).map_err(|e| format!("Malformed compiler graph: {e}"))?;
    if graph.crate_name != crate_name { return Err("Collector returned the wrong crate".into()); }
...
fn assert_forbidden(name: &str, graph: &Graph) -> TestResult {
    ...
    let findings = FIXTURE.check(graph).expect_err("Outer dependency was missed");
    ...
    fs::write(root().join(".test-artifacts/resolved-layers").join(name).join("result.txt"), "PASS: expected resolved dependency rejected\n")?;
    Ok(())
}
...
resolved_violation!(fully_qualified_constructor,
    "pub fn leak() { let _ = crate::infrastructure::Repo::new(); }", "");
resolved_violation!(fully_qualified_return_type,
    "pub fn leak() -> crate::infrastructure::Repo { crate::infrastructure::Repo }", "");
resolved_violation!(unused_import,
    "pub use crate::infrastructure::Repo;", "");
resolved_violation!(grouped_aliased_import,
    "pub use crate::infrastructure::{Repo as Storage, Port};", "");
resolved_violation!(glob_import,
    "pub use crate::infrastructure::*;", "");
resolved_violation!(root_reexport,
    "pub use crate::facade::Alias;", "pub use crate::infrastructure::Repo as Alias;");
...
resolved_violation!(nested_module_and_relative_path,
    "pub mod nested { pub use super::super::infrastructure::Repo; }", "");

#[test]
fn actual_single_crate_obeys_layers() -> TestResult {
    let graph = analyze("actual_single_crate", &root().join("src/lib.rs"), "clean_architecture", &[])?;
    report("actual_single_crate", &CLEAN, &graph)?;
    assert_eq!(CLEAN.check(&graph), Ok(()));
    assert!(graph.edges.iter().any(|e| e.from.ends_with("::application") && e.to.ends_with("::domain")), "Collector silently missed real references");
    Ok(())
}

#[test]
fn permitted_directions_and_local_names_pass() -> TestResult {
    let graph = run_source("permitted_directions", &fixture(
        "pub struct Repo; pub fn pure() -> Repo { Repo }", "pub use crate::infrastructure::Repo;"
    ), &[])?;
...
fn feature_specific_reference_is_checked_when_enabled() -> TestResult {
    let source = fixture("#[cfg(feature = \"leak\")] pub fn leak() { let _ = crate::infrastructure::Repo::new(); }", "");
...
fn new_module_requires_classification() -> TestResult {
    ...
    let findings = FIXTURE.check(&graph).expect_err("Unclassified module passed");
...
fn permitted_edges_can_still_form_a_forbidden_cycle() -> TestResult {
    ...
    assert!(findings.iter().any(|f| matches!(f, Finding::Cycle(path) if path.len() == 3)));
    assert!(!findings.iter().any(|f| matches!(f, Finding::Forbidden { .. })));
    let permitted = Policy { acyclic: false, ..CYCLE };
