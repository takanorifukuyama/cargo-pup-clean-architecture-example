...
pub fn find_rule<'a>(rules: &'a [Rule], name: &str) -> TestResult<&'a Rule> {
    ...
    let rule = matches.next().ok_or_else(|| format!("Unknown rule: {name}"))?;
...
fn configuration(rule: &Rule) -> TestResult<String> {
    ...
    let denied = rule.deny_imports.iter()
...
fn setting<'a>(text: &'a str, key: &str) -> TestResult<&'a str> {
    ...
    if value.is_empty() || !value.chars().all(|ch| ch.is_ascii_alphanumeric() || "-._".contains(ch)) {
    ...
    let valid = match expected {
        ...
        Expectation::Denied => {
            code.is_some_and(|value| value > 0)
                && [rule, "Use of module", "is denied"].iter().all(|token| output.contains(token))
...
impl Project {
    fn new() -> TestResult<Self> {
        ...
        let path = std::env::temp_dir().join(format!("pup-rust-{}-{time}-{id}", std::process::id()));
...
fn execute(command: &mut Command, log: &Path) -> TestResult<(Option<i32>, String)> {
    ...
    command.stdout(Stdio::from(file.try_clone()?)).stderr(Stdio::from(file));
    command.env("CARGO_TERM_COLOR", "never").env("NO_COLOR", "1");
    // Do not inherit an outer cargo test's target directory / rustc wrapper.
    for name in ["CARGO_TARGET_DIR", "CARGO_BUILD_TARGET_DIR", "RUSTC_WRAPPER", "RUSTC_WORKSPACE_WRAPPER"] {
...
fn run(root: &Path, logs: &Path, name: &str, rule: &Rule, edit: Option<Edit>, expected: Expectation) -> TestResult {
    let pins = include_str!("../../pup-toolchain.toml");
    let version = setting(pins, "version")?;
    let toolchain = setting(pins, "toolchain")?;
    let pup = root.join(".tools/cargo-pup/bin").join(format!("cargo-pup{}", std::env::consts::EXE_SUFFIX));
    if !pup.is_file() {
        return Err("cargo-pup is not installed. Run: python3 scripts/setup_pup.py".into());
    }
    let (code, output) = execute(Command::new(&pup).arg("--version"), &logs.join(format!("{name}.version.log")))?;
    ...
    if let Some(edit) = edit {
        ...
        if !path.components().all(|part| matches!(part, Component::Normal(_))) || !path.starts_with("src") {
            return Err(format!("Mutation must name a relative file under src/: {}", edit.file).into());
        }
